...
class DataGenerator():
    ...
    def _get_data_predict(self):
        ...
        assets_states[0, :, :, 0] = tmp_states[:, 1:, -1, 0] / tmp_states[:, :-1, -1, 0]
        assets_states[0, :, :, 1] = np.nanmax(tmp_states[:, 1:, :, 1], axis=-1) / tmp_states[:, 1:, -1, 0]
        assets_states[0, :, :, 2] = np.nanmin(tmp_states[:, 1:, :, 2], axis=-1) / tmp_states[:, 1:, -1, 0]
