...
from .environment.portfolio_env import Reward
...
class RLAgent():
    ...
    def train_episode(self):
        """
        Train one episode
        :return: episode_return, avg_rho, avg_mdd, episode_loss
        """
        ...
        while True:
            ...
            std = torch.std(ror, dim=-1, keepdim=True)
            ...
            asu_grad = torch.clamp(asu_grad, min=self.eps)
            ...
            if done:
                if self.args.msu_bool:
                    steps_log_p_rho = torch.nan_to_num(steps_log_p_rho, nan=0.0, posinf=0.0, neginf=0.0)
                ...
                std_rt = torch.std(rewards_total, dim=-1, keepdim=True)
                rewards_total = (rewards_total - rewards_total.mean(-1, True)) / (std_rt + self.eps)

                rewards_mdd = - 2 * torch.from_numpy(mdd - 0.5).to(self.args.device)
                rewards_mdd = torch.clamp(rewards_mdd, min=-1e6, max=1e6)
    ...
    def cal_MDD(self, agent_wealth):
        ...
        drawdown = (np.maximum.accumulate(agent_wealth, axis=-1) - agent_wealth) / denom
